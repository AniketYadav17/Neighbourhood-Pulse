...
# Current stable flash-lite tier open to new users (docs, 2026-07-13); its two
# predecessors broke within a week (daily quota, then withdrawal for new users),
# so the model is also a runtime lever: `pulse briefs --model NAME`, and a 404
# on the configured model aborts the run with guidance instead of looping.
BRIEFS_MODEL = "gemini-3.1-flash-lite"
# gemini-3.x models take thinking_level (no full-off value exists in the family;
# "minimal" is the floor). Thinking tokens share max_output_tokens with the
# JSON completion, hence the generous BRIEFS_MAX_TOKENS below.
BRIEFS_THINKING_CONFIG = {"thinking_level": "minimal"}
BRIEFS_MAX_TOKENS = 4000  # headroom for hidden thinking tokens; the brief itself is ~200
BRIEFS_TEMPERATURE = 0.2
BRIEFS_MAX_COST_USD = 1.00  # hard stop; a full run is well under $0.01 on the paid tier
# Paid-tier text pricing (docs, 2026-07-13); the free tier is $0.
BRIEFS_PRICE_PER_MTOK = {"input": 0.25, "output": 1.50}
# Observed free-tier pacing (10 RPM +10% margin, measured on the predecessor;
# Google publishes no static per-model table). A wrong guess degrades to
# "slower than necessary" — per-minute 429s retry, daily-quota 429s stop the
# run cleanly with progress saved. Set to 0 on a paid tier.
